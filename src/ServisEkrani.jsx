...
// Cep telefonu mu? (TR mobil = +90 5XX…). WhatsApp YALNIZ cep numaralarında gösterilir.
const cepMi = (tel) => {
  const d = String(tel || "").replace(/\D/g, "").replace(/^90/, "").replace(/^0/, "");
  return d.length === 10 && d[0] === "5";
};
const waLink = (tel, mesaj) => {
  let d = String(tel || "").replace(/\D/g, "").replace(/^0/, "");
  if (!d.startsWith("90")) d = "90" + d;
  return `https://wa.me/${d}${mesaj ? `?text=${encodeURIComponent(mesaj)}` : ""}`;
};
const WaIkon = () => (
...
function ServisKarti({ servis, onSec, cihaz }) {
  return (
    <div
