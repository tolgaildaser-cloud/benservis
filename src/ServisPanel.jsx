...
const DURUM_LABEL = {
  ...
  reddedildi: { label: "Reddedildi", color: "#DC2626" },
  suresi_doldu: { label: "Süresi Doldu", color: "#64748B" },
    ...
    <div style={{ minHeight: "100vh", background: CREAM, fontFamily: "'Hanken Grotesk', sans-serif", padding: "32px 18px" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <input
          ...
          style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: `1.5px solid ${secilenId ? GREEN : "#E2E8F0"}`, background: "#F8FAFC", fontSize: 14, fontFamily: "inherit", outline: "none", marginBottom: 8 }}
        />
        {aramaMetni && !secilenId && (
          <div style={{ border: "1px solid #E2E8F0", borderRadius: 10, background: "#F8FAFC", maxHeight: 260, overflowY: "auto", marginBottom: 12 }}>
            {filtreliServisler.length === 0 && (
              <div style={{ padding: "12px 14px", fontSize: 13, color: "#94A3B8" }}>Servis bulunamadı</div>
            )}
            {filtreliServisler.map(s => (
              <div
                key={s.id}
                onClick={() => sec(s)}
                style={{ padding: "11px 14px", fontSize: 13, borderBottom: "1px solid #F1F5F9", cursor: "pointer" }}
        ...
        {secilenId && (
          <div style={{ padding: "10px 13px", borderRadius: 9, background: "#F1F5F9", border: `1px solid ${GREEN}`, marginBottom: 12, fontSize: 13 }}>
            ✓ <strong>{secilenAd}</strong><br />
            <span style={{ fontSize: 11, color: "#475569" }}>{secilenId}</span>
          </div>
        )}
        {hata && <div style={{ color: "#DC2626", fontSize: 13, marginBottom: 10 }}>{hata}</div>}
        <button
          onClick={kaydet}
          disabled={yukleniyor || !secilenId}
          style={{ width: "100%", padding: 13, borderRadius: 11, border: "none", background: secilenId ? AMBER : "#CBD5E1", color: "#fff", fontWeight: 700, fontSize: 15, cursor: secilenId ? "pointer" : "not-allowed" }}>
      ...
      <div style={{ background: "#1E293B", borderRadius: 18, padding: 32, border: "1px solid #334155", boxShadow: "0 18px 48px -24px rgba(0,0,0,.55)", width: "100%", maxWidth: 360 }}>
        ...
        <form onSubmit={girisYap}>
          ...
          <button
            ...
            disabled={yukleniyor}
            style={{ width: "100%", marginTop: 10, padding: 8, borderRadius: 8, border: "none", background: "none", color: "#94A3B8", fontSize: 12.5, cursor: "pointer", textDecoration: "underline" }}
          >
            Şifremi unuttum
          </button>
        </form>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: CREAM, borderRadius: 16, padding: 24, width: "92%", maxWidth: 400, maxHeight: "85vh", overflowY: "auto", fontFamily: "'Hanken Grotesk', sans-serif" }}>
        ...
        <div style={{ fontSize: 12, color: "#64748B", marginBottom: 16 }}>#{is.is_no}</div>
        ...
        <textarea
          ...
          rows={2}
          style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1.5px solid #E2E8F0", fontSize: 13, fontFamily: "'Hanken Grotesk', sans-serif", boxSizing: "border-box", resize: "vertical", marginBottom: 14 }}
        />
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
          <input
            ...
            style={{ flex: 1, padding: "9px 12px", borderRadius: 8, border: "1.5px solid #E2E8F0", fontSize: 13, fontFamily: "'Hanken Grotesk', sans-serif" }}
          />
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {parcalar.map(p => (
              <button key={p} type="button" onClick={() => setParcalar(prev => prev.filter(x => x !== p))}
                style={{ fontSize: 12, background: "#E2E8F0", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer" }}>
                {p} ✕
              </button>
            ))}
          </div>
        ...
        <input
          ...
          placeholder="3200"
          style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1.5px solid #E2E8F0", fontSize: 13, fontFamily: "'Hanken Grotesk', sans-serif", boxSizing: "border-box", marginBottom: 14 }}
        />

        <label style={{ fontSize: 12, fontWeight: 700, color: INK, display: "block", marginBottom: 6 }}>
          Fotoğraf <span style={{ fontWeight: 400, color: "#64748B" }}>(öncesi/sonrası, max 5)</span>
        </label>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          {fotograflar.map((url, i) => (
            <div key={url} style={{ position: "relative" }}>
              <img src={url} alt={`foto ${i+1}`} style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 6, border: "1px solid #E2E8F0" }} />
              <button type="button" onClick={() => setFotograflar(prev => prev.filter(u => u !== url))}
                style={{ position: "absolute", top: -6, right: -6, background: "#DC2626", color: "#fff", border: "none", borderRadius: "50%", width: 16, height: 16, fontSize: 9, cursor: "pointer" }}>✕</button>
            </div>
          ))}
          {fotograflar.length < 5 && (
            <button type="button" onClick={() => inputRef.current?.click()} disabled={fotoYukleniyor}
              style={{ width: 56, height: 56, border: "1.5px dashed #E2E8F0", borderRadius: 6, background: "#F8FAFC", color: "#94A3B8", fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {fotoYukleniyor ? "⏳" : "+"}
            </button>
          )}
        </div>
        ...
        {hata && <div style={{ color: "#DC2626", fontSize: 12, marginBottom: 12 }}>{hata}</div>}

        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onKapat} type="button"
            style={{ flex: 1, padding: 11, borderRadius: 10, border: "1.5px solid #E2E8F0", background: "white", fontSize: 13, cursor: "pointer" }}>
            İptal
          </button>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: CREAM, borderRadius: 16, padding: 24, width: "90%", maxWidth: 360, fontFamily: "'Hanken Grotesk', sans-serif" }}>
        ...
        <input
          ...
          style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1.5px solid #E2E8F0", fontSize: 14, fontFamily: "'Hanken Grotesk', sans-serif", boxSizing: "border-box", marginBottom: 16 }}
        />
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onKapat} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1.5px solid #E2E8F0", background: "white", fontSize: 14, cursor: "pointer" }}>
            İptal
          </button>
  ...
  return (
    <div style={{ background: "white", border: `1.5px solid ${AMBER}`, borderRadius: 12, padding: 14, marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        ...
        {kalan !== null && (
          <span style={{ fontSize: 11, color: kalan < 5 ? "#DC2626" : AMBER, fontWeight: 700 }}>
            ⏱ {kalan} dk kaldı
          </span>
        )}
      </div>
...
function IsKarti({ is, jwtToken, onGuncelle }) {
  ...
  const { label, color } = DURUM_LABEL[is.durum] || { label: is.durum, color: "#64748B" };

  const qrIndir = async () => {
    const url = `https://benservis.com/dpp/${encodeURIComponent(is.seri_no)}`;
    const canvas = document.createElement("canvas");
    await QRCode.toCanvas(canvas, url, { width: 300, margin: 2, color: { dark: "#1E293B", light: "#F1F5F9" } });
  ...
  return (
    <>
      ...
      <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 12, padding: 16, marginBottom: 10 }}>
        ...
        {is.durum === "bekliyor" && (
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            ...
            <button onClick={() => islemYap("ret")} disabled={yukleniyor}
              style={{ flex: 1, padding: 9, borderRadius: 8, border: "1.5px solid #DC2626", background: "white", color: "#DC2626", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              ✕ Reddet
            </button>
          </div>
        )}
        ...
        {is.durum === "tamamlandi" && (
          <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 8, background: "#F1F5F9", fontSize: 12 }}>
            {dppZenginlesti ? (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {is.seri_no && (
                    <button
                      onClick={qrIndir}
                      style={{ padding: "5px 10px", borderRadius: 6, border: `1.5px solid #1E293B`, background: "white", color: "#1E293B", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>
                      📷 QR
                    </button>
                  )}
                  <button
              ...
              <span style={{ color: "#94A3B8" }}>— Seri no girilmedi, DPP kaydı yok</span>
            )}
            {is.seri_no && (
              <div style={{ marginTop: 10, borderTop: "1px solid #E2E8F0", paddingTop: 8, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, color: "#64748B" }}>Cihaz durumu:</span>
                <select
                  value={cihazDurum}
                  onChange={e => durumGuncelle(e.target.value)}
                  disabled={durumYukleniyor}
                  style={{ fontSize: 11, padding: "4px 8px", borderRadius: 6, border: "1px solid #E2E8F0", background: "#F8FAFC", cursor: durumYukleniyor ? "wait" : "pointer", fontFamily: "'Hanken Grotesk', sans-serif" }}>
            ...
            {is.puan !== null && is.puan !== undefined && (
              <div style={{ marginTop: 8, borderTop: "1px solid #E2E8F0", paddingTop: 8 }}>
                <div style={{ fontSize: 11, color: "#64748B", marginBottom: 2 }}>Müşteri Değerlendirmesi</div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 16 }}>{"⭐".repeat(is.puan)}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GREEN }}>{is.puan}/5</span>
                </div>
                {is.yorum && <div style={{ fontSize: 12, color: "#475569", marginTop: 4, fontStyle: "italic" }}>"{is.yorum}"</div>}
              </div>
            )}
          </div>
        )}
...
function UrunlerTab({ session }) {
  ...
  const inp = { width: "100%", padding: "10px 12px", borderRadius: 8, border: "1.5px solid #E2E8F0", fontSize: 14, fontFamily: "'Hanken Grotesk', sans-serif", boxSizing: "border-box", marginBottom: 12 };
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <button
          ...
          style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: formAcik ? "#E2E8F0" : AMBER, color: formAcik ? INK : "white", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
      ...
      {/* Ürün ekleme formu */}
      {formAcik && (
        <form onSubmit={ekle} style={{ background: "white", borderRadius: 12, padding: 16, marginBottom: 16, border: "1.5px solid #E2E8F0" }}>
          ...
          <label style={{ fontSize: 12, fontWeight: 700, color: INK, display: "block", marginBottom: 4 }}>
            Ürün Fotoğrafı <span style={{ fontWeight: 400, color: "#64748B" }}>(önerilir)</span>
          </label>
          <div style={{ marginBottom: 12 }}>
            {form.gorsel_url ? (
              <div style={{ position: "relative", display: "inline-block" }}>
                <img src={form.gorsel_url} alt="ürün" style={{ width: 96, height: 96, objectFit: "cover", borderRadius: 10, border: "1.5px solid #E2E8F0" }} />
                <button type="button" onClick={() => setForm(f => ({ ...f, gorsel_url: "" }))}
                  style={{ position: "absolute", top: -8, right: -8, background: "#DC2626", color: "#fff", border: "none", borderRadius: "50%", width: 20, height: 20, fontSize: 10, cursor: "pointer" }}>✕</button>
              </div>
            ) : (
              <button type="button" onClick={() => fotoInputRef.current?.click()} disabled={fotoYuk}
                style={{ width: 96, height: 96, border: "1.5px dashed #E2E8F0", borderRadius: 10, background: "#F8FAFC", color: "#94A3B8", fontSize: 12, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, fontFamily: "'Hanken Grotesk', sans-serif" }}>
          ...
          <label style={{ fontSize: 12, fontWeight: 700, color: INK, display: "block", marginBottom: 4 }}>
            Seri No <span style={{ fontWeight: 400, color: "#64748B" }}>(opsiyonel — DPP bağlantısı için)</span>
          </label>
          ...
          <div style={{ fontSize: 11, color: "#94A3B8", marginBottom: 14 }}>Seri no girilirse alıcı DPP tamir geçmişini görebilir.</div>
      ...
      {/* Ürün listesi */}
      {yukleniyor ? (
        <p style={{ textAlign: "center", color: "#64748B", fontSize: 13 }}>Yükleniyor...</p>
      ) : urunler.length === 0 ? (
        ...
        urunler.map(u => (
          <div key={u.id} style={{ background: "white", borderRadius: 12, padding: 14, marginBottom: 10, border: "1px solid #E2E8F0" }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              {u.gorsel_url && (
                <img src={u.gorsel_url} alt={u.baslik} style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8, border: "1px solid #E2E8F0", flexShrink: 0 }} />
              )}
              <div style={{ flex: 1 }}>
                ...
                {u.aciklama && <div style={{ fontSize: 12, color: "#475569", marginTop: 4 }}>{u.aciklama}</div>}
                {u.dpp_seri_no && (
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              ...
              <button
                ...
                style={{ flex: 1, padding: 8, borderRadius: 8, border: "1.5px solid #64748B", background: "white", color: "#64748B", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                Pasife Al
              </button>
              <button
                onClick={() => sil(u.id)}
                style={{ padding: "8px 12px", borderRadius: 8, border: "1.5px solid #DC2626", background: "white", color: "#DC2626", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                ✕
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'Hanken Grotesk', sans-serif" }}>
      ...
      <div style={{ padding: 16, maxWidth: 600, margin: "0 auto" }}>
        ...
        <div style={{ fontSize: 11, color: "#94A3B8", background: "#F1F5F9", borderRadius: 8, padding: "6px 10px", marginBottom: 10, wordBreak: "break-all" }}>
        ...
        {saatStr && (
          <p style={{ textAlign: "right", fontSize: 11, color: "#94A3B8", margin: "0 0 10px" }}>
        ...
        {aktifTab === "isler" && (
          <>
            {yukleniyor && isler.length === 0 && <p style={{ textAlign: "center", color: "#64748B" }}>Yükleniyor...</p>}
            {listeHata && (
            ...
            {havuzIsler.length > 0 && (
              <>
                ...
                <div style={{ borderBottom: "1px solid #E2E8F0", margin: "14px 0" }} />
              </>
            )}
            ...
            {digerler.length > 0 && (
              <>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#64748B", margin: "16px 0 8px" }}>Geçmiş</div>
                {digerler.map(is => (
                  <IsKarti key={is.id} is={is} jwtToken={session.access_token} onGuncelle={onGuncelle} />
                ))}
              </>
            ...
            {!yukleniyor && isler.length === 0 && (
              <p style={{ textAlign: "center", color: "#64748B", marginTop: 40 }}>Henüz talep yok.</p>
            )}
          </>
        )}
