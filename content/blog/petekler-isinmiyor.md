...
faq:
  ...
  - q: "Üst kat ısınmıyor, alt kat sıcaksa ne yapmalı?"
    a: "Genelde tesisat basıncı düşüktür. Kombi soğukken manometre 1-1,5 bar aralığında olmalıdır; basınç düşünce su üst kata çıkamaz. Hava aldıktan sonra basınç düşer, bu normaldir — doldurma musluğuyla aralığa getirilir. Basınç sürekli düşüyorsa tesisatta kaçak vardır ve servis gerekir."
images:
...
> ⚡ **Kısa özet:** Peteğin **üstü soğuk altı sıcaksa** içinde hava vardır — hava alma vidasından boşaltılır, ücretsizdir. **Altı soğuk üstü sıcaksa** dipte tortu birikmiştir, bu servis işidir. **Hiçbir petek ısınmıyorsa** su dolaşmıyordur: pompa, vana ya da oda termostatı. **Üst kat soğuksa** basınca bak — kombi soğukken **1-1,5 bar**.
...
| Üst kat soğuk, alt kat sıcak | Tesisat basıncı düşük | Kendin, ücretsiz |
...
📌 **Hava aldıktan sonra basınç düşer, bu beklenen bir sonuçtur.** Sistemden hava çıktı, yerine su gerekiyor. Manometreye bak ve aşağıdaki aralığa getir.

## Basınç: kombi soğukken 1-1,5 bar

Manometre kombinin ön yüzündedir. Doğru okuma **kombi kapalı ve tesisat soğukken** yapılır; çalışırken su genleşir ve iğne 2 bara doğru çıkabilir, bu normaldir.

- **1 barın altı:** su üst katlara çıkamaz. Alt kat ısınır, üst kat soğuk kalır. Doldurma musluğu açılarak aralığa getirilir.
- **2,5 barın üstü:** fazla dolmuştur; emniyet ventilinden su boşaltmak gerekir, bu genelde servis işidir.

🔴 **Basınç sürekli düşüyorsa mesele doldurmak değildir.** Her hafta yeniden dolduruyorsan tesisatta ya da kombide bir kaçak vardır. Sürekli su takviyesi kireç ve korozyonu hızlandırır — kaçağın kaynağı bulunmalıdır.
...
5. Manometreye bak, kombi soğukken 1-1,5 bar aralığına getir.
...
Evet. Havanın yerini su alacak — kombi soğukken 1-1,5 bar aralığına getir.
