---
title: "Çamaşır makinesi su atmıyor: nedenleri ve çözümü"
description: "Çamaşır makinen suyu atmıyor, çamaşırlar ıslak mı çıkıyor? Tahliye filtresi, pompa ve hortum kontrolü + ücretsiz maliyet tahmini. Bil, gör, çağır."
slug: "camasir-makinesi-su-atmiyor"
date: "2026-06-17"
category: "Çamaşır makinesi"
faq:
  - q: "Çamaşır makinesi su atmıyor, ilk ne kontrol edilir?"
    a: "Önce alt kapaktaki tahliye filtresine bak. Makine suyu boşaltamazsa son su atma adımına geçemez; en sık sebep tıkalı filtredir."
  - q: "Çamaşırlar ıslak çıkıyor / su boşaltmıyor, neden?"
    a: "Genelde tahliye filtresi veya pompası tıkalı/arızalıdır, ya da tahliye hortumu bükük/tıkalıdır."
  - q: "Dengesiz yük su atmayı durdurur mu?"
    a: "Evet. Tek parça ağır çamaşır (battaniye, halı) topaklanıp dengesizlik yapar; makine güvenlik için su atma adımını azaltır veya durdurur, çamaşır ıslak çıkar. Yükü dağıt."
  - q: "Tamir mi yenisi mi?"
    a: "Filtre/pompa/hortum ucuz, tamir mantıklı. Motor veya elektronik kart pahalıysa ve makine eskiyse yenisi değerlendirilebilir."
images:
  coverAlt: "Ön yüklemeli çamaşır makinesinin cam kapağı ardında, tahliye edilmemiş suyun içinde duran ıslak çamaşırlar"
---

Yıkama bitti ama çamaşırlar sırılsıklam — makine suyu atamamış. Sinir bozucu ama çoğu zaman sebep basit ve evde, dakikalar içinde çözülüyor. Bu yazıda nedenleri, **servis çağırmadan önce kendin yapabileceğin kontrolleri** ve iş servise kaldığında ne beklemen gerektiğini bulacaksın. Cihazına özel tahmini maliyeti benservis.com'daki ücretsiz teşhisten alabilirsin.

**Marka fark etmez:** Arçelik, Beko, Bosch, Siemens, Samsung, LG, Profilo gibi makinelerde nedenler benzerdir. Ekranda hata kodu varsa markana özel rehber: [Bosch](/blog/bosch-camasir-makinesi-hata-kodlari/) · [Arçelik](/blog/arcelik-camasir-makinesi-hata-kodlari/) · [marka marka](/blog/camasir-makinesi-hata-kodlari/).

## Çamaşır makinesi neden su atmaz, çamaşırı ıslak bırakır?

**1. Tıkalı tahliye filtresi (en sık).** Alt ön kapaktaki filtre tüy, bozuk para, düğme ile tıkanır; makine suyu boşaltamayınca son adıma geçemez, çamaşırlar ıslak kalır.

**2. Tahliye pompası arızası.** Pompa çalışmazsa su gitmez, çamaşır ıslak kalır. Pompadan anormal ses gelebilir.

**3. Bükük veya tıkalı tahliye hortumu.** Arkadaki hortum sıkışmış ya da içi tıkanmış olabilir.

**4. Dengesiz yük.** Tek parça ağır çamaşır topaklanır; makine güvenlik için son su atma adımını azaltır/durdurur, çamaşır ıslak çıkar.

**5. Kayış veya motor (özellikle eski modeller).** Kayış gevşemiş/kopmuş ya da motor kömürleri bitmiş olabilir.

## Servisi aramadan önce kendin kontrol et

1. **Tahliye filtresini temizle.** Alt ön kapağı aç, önce bir kap/havlu hazırla (su gelir), filtreyi çevirip çıkar, içindeki tüy/parçayı temizle, tak. *(Adım adım: [tahliye filtresi nasıl temizlenir](/blog/camasir-makinesi-tahliye-filtresi-temizleme/).)*
2. **Tahliye hortumunu kontrol et.** Arkada bükük/ezik mi, gideri tıkalı mı bak.
3. **Yükü dengele.** Tek ağır parça varsa böl, programı tekrar dene.
4. **Makineyi resetle.** Fişi 1 dakika çek, tekrar dene.

## Ne zaman mutlaka servis çağırmalısın?

- Filtre temiz, hortum açık olduğu hâlde **hâlâ su atmıyor / çamaşır ıslak kalıyorsa** (pompa)
- Pompadan **ses geliyor ama çalışmıyorsa**
- Tamburdan **gürültü / kayış kokusu** geliyorsa (kayış/motor)

## Tamir maliyeti ne kadar olur?

Bu arızanın maliyeti markaya, modele ve gerçek arızaya göre değişir; bu sayfada aralık vermiyoruz. **[Cihazına göre tahmini maliyeti ücretsiz öğren →](/)** Belirtiyi yaz, olası arızayı ve tahmini maliyeti saniyede gör.
## Sık sorulan sorular

**Çamaşır makinesi su atmıyor, ilk ne kontrol edilir?**
Alt kapaktaki tahliye filtresi. Makine suyu boşaltamazsa son adıma geçemez; en sık sebep tıkalı filtredir.

**Çamaşırlar ıslak çıkıyor, neden?**
Tahliye filtresi/pompası tıkalı veya hortum bükük olabilir.

**Dengesiz yük su atmayı durdurur mu?**
Evet, tek ağır parça topaklanıp dengesizlik yapar; yükü dağıt.

**Tamir mi yenisi mi?**
Filtre/pompa/hortum ucuz, tamir mantıklı. Motor/kart pahalıysa ve makine eskiyse yenisi değerlendirilebilir.
