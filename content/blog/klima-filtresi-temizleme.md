---
title: "Klima filtresi nasıl temizlenir? (adım adım)"
description: "Klima soğutmuyor, kokuyor ya da az üflüyorsa en sık sebep kirli filtre. 6 adımda kendin temizle — ücretsiz, ~15 dakika. Bil, gör, çağır."
slug: "klima-filtresi-temizleme"
date: "2026-06-21"
category: "Klima"
guide:
  difficulty: "Kolay"
  time: "~15 dakika"
  totalTime: "PT15M"
  cost: "Ücretsiz"
  tools: ["Ilık su", "Yumuşak fırça / sünger", "Kuru bez"]
steps:
  - "Klimayı kapat."
  - "İç ünitenin ön kapağını yukarı doğru nazikçe kaldır."
  - "Izgara filtreleri yuvasından yavaşça çıkar."
  - "Ilık suyla durula; çok kirliyse yumuşak fırçayla nazikçe ov. Sıcak su ve deterjan kullanma."
  - "Filtreleri gölgede, tamamen kuruyana kadar beklet."
  - "Filtreleri yerine tak ve ön kapağı kapat."
# GRF adım görselleri (2 Ağu 2026) — dosyalar public/tamir/klima-filtresi-temizleme/.
# Sıra ÖNEMLİ: images.steps[N-1] ↔ adim-0N.png ↔ yukarıdaki steps[N-1].
images:
  coverAlt: "Klima filtresi temizliği rehberinin kapak çizimi"
  steps:
    - "Klimanın kumandayla kapatılışını gösteren çizim"
    - "Klima iç ünitesinin ön kapağının iki yandan tutulup yukarı kaldırılışını gösteren çizim"
    - "Izgara filtrelerin klima iç ünitesinden yavaşça çıkarılışını gösteren çizim"
    - "Klima filtresinin ılık suyla durulanışını, sıcak su ve deterjanın kullanılmayacağını gösteren çizim"
    - "Klima filtresinin gölgede, doğrudan güneş almadan kurutuluşunu gösteren çizim"
    - "Kuruyan klima filtresinin yerine takılıp ön kapağın kapatılışını gösteren çizim"
faq:
  - q: "Klima filtresini ne sıklıkla temizlemeli?"
    a: "Yoğun kullanımda 2-3 haftada bir, normalde ayda bir. Sezon başında mutlaka temizle; en sık 'klima soğutmuyor' şikâyetinin sebebi kirli filtredir."
  - q: "Filtreyi sıcak suyla yıkayabilir miyim?"
    a: "Hayır. Sıcak su ve deterjan filtreye zarar verebilir; ılık su ve yumuşak fırça yeterli. İyice durula."
  - q: "Islak takarsam ne olur?"
    a: "Islak filtre küf ve koku yapar. Mutlaka tamamen kuruyana kadar (gölgede) beklet, sonra tak."
  - q: "Temizledim ama yine soğutmuyor?"
    a: "O zaman sorun gaz, kompresör ya da dış ünitede olabilir; bu servis işidir. Olası arızayı ve tahmini maliyeti Benservis'ten öğren."
---

Klima **soğutmuyor**, **kötü kokuyor** ya da **zayıf üflüyorsa**, en sık sebep şaşırtıcı derecede basittir: **kirli hava filtresi**. Tıkalı filtre hava akışını boğar, soğutmayı düşürür ve koku yapar. İyi haber — bunu **kendin, ücretsiz, ~15 dakikada** temizleyebilirsin. İşte adım adım.

## Adım adım: klima filtresi temizliği

**1. Klimayı kapat.** Çalışırken müdahale etme.

**2. Ön kapağı aç.** İç ünitenin önündeki kapağı **yukarı/dışarı** doğru nazikçe kaldır (çoğu modelde iki yandan tutup açılır).

**3. Filtreleri çıkar.** İçerideki ızgara filtreleri yuvasından **yavaşça** çek.

**4. Yıka.** **Ilık suyla** durula; çok kirliyse yumuşak fırça/süngerle nazikçe ov. **Sıcak su ve deterjan kullanma** — filtreye zarar verir.

**5. Kurut.** Filtreleri **gölgede**, tamamen kuruyana kadar beklet. (Islak takma → küf ve koku yapar.)

**6. Tak.** Filtreleri yerine oturt, ön kapağı kapat. Sezon boyunca birkaç haftada bir tekrarla.

## Ne zaman servis gerekir?
Filtre tertemiz, ayarlar doğru olduğu hâlde klima **hâlâ soğutmuyorsa**, sorun **gaz, kompresör ya da dış ünitede** olabilir — bu uzmanlık ister, kendin müdahale etme.

İlgili: [Klima soğutmuyor: 6 olası neden](/blog/klima-sogutmuyor-nedenleri/) · [Klima bakımı kaç para?](/blog/klima-bakimi-kac-para/) · [Klimadan koku geliyor](/blog/klima-koku-yapiyor/)

## Sık sorulan sorular

**Ne sıklıkla temizlemeli?**
Yoğun kullanımda 2-3 haftada bir, normalde ayda bir; sezon başında mutlaka.

**Sıcak suyla yıkayabilir miyim?**
Hayır — ılık su + yumuşak fırça. Sıcak su/deterjan zarar verir.

**Islak takarsam?**
Küf ve koku yapar; tamamen kuruyana kadar beklet.

**Temizledim ama yine soğutmuyor?**
Gaz/kompresör/dış ünite olabilir; servis gerekir. Tahmini maliyeti Benservis'ten al.
