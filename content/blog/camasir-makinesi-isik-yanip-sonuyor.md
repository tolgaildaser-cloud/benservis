---
title: "Çamaşır makinesi ışıkları yanıp sönüyor: hata ne demek?"
description: "Çamaşır makinende ışıklar yanıp sönüyor ya da hata kodu mu var? En sık sebepler ve ne zaman servis gerekir. Bil, gör, çağır."
slug: "camasir-makinesi-isik-yanip-sonuyor"
date: "2026-06-19"
category: "Çamaşır makinesi"
faq:
  - q: "Çamaşır makinesinde ışıklar neden yanıp söner?"
    a: "Yanıp sönen ışık genelde bir hata kodudur: makine bir sorunu (kapı kilidi, su almama/atmama, ısıtma veya denge) ışık deseniyle anlatır. Hangi ışığın kaç kez yandığı arızayı işaret eder."
  - q: "Yanıp sönen ışığı kendim çözebilir miyim?"
    a: "Bazılarını evet: kapağı tam kapat, tahliye filtresini temizle, su musluğunu aç, makineyi fişten çekip resetle. Desen geçmiyorsa kodun anlamına göre servis gerekir."
  - q: "Hata kodunun anlamını nasıl öğrenirim?"
    a: "Kullanım kılavuzundaki kod tablosuna bak ya da belirtiyi (hangi ışık, kaç kez) Benservis'e yaz; olası arızayı ve tahmini maliyeti saniyede söyler."
  - q: "Tamiri kaç para?"
    a: "Arızaya göre değişir: kapı kilidi, su valfi/pompa ve elektronik kart çok farklı işlerdir. Kesin tahmini cihazına göre Benservis'ten ücretsiz al."
images:
  checks:
    - "Çamaşır makinesi kapağının kilidi oturana kadar itilip tam kapatılışını gösteren çizim"
    - "Su musluğunun açık olup olmadığının ve giriş süzgecinin tıkalı olup olmadığının kontrolünü gösteren çizim"
    - "Alt ön kapaktaki tahliye filtresinin çıkarılıp tüy ve ipliğin temizlenişini gösteren çizim"
    - "Tamburdaki tek ağır parça ile bölünüp dağıtılmış yükün karşılaştırıldığı çizim"
    - "Fişin bir dakika çekilişini ve çocuk kilidinin kontrolünü gösteren çizim"
    - "Panelde yanıp sönen ışık deseninin not alınmasını gösteren çizim"
  coverAlt: "Ankastre nişe yerleştirilmiş çamaşır makinesinin kapağı ve tamburu yakın plan"
---

Çamaşır makinesi çalışmıyor ve ekranındaki ya da düğmelerindeki ışıklar **yanıp sönüyor** — bu aslında makinenin sana "bir sorun var" demesidir. Yanıp sönme rastgele değil; çoğu modelde bir **hata kodudur** ve hangi ışığın kaç kez yandığı arızayı işaret eder. Bu yazıda en sık sebepleri, **kendin çözebileceklerini** ve hangi kodun servis işi olduğunu anlatıyoruz. Cihazına özel tahmini maliyeti benservis.com'daki ücretsiz teşhisten alabilirsin.

**Marka fark etmez:** Arçelik, Beko, Bosch, Siemens, Samsung, LG, Profilo gibi makinelerde nedenler benzerdir. Yanıp sönen ışık bir hata kodu olabilir — markana özel rehber: [Bosch](/blog/bosch-camasir-makinesi-hata-kodlari/) · [Arçelik](/blog/arcelik-camasir-makinesi-hata-kodlari/) · [marka marka](/blog/camasir-makinesi-hata-kodlari/).

## Işık yanıp sönmesi ne anlama gelir? Olası nedenler

**1. Kapı/kapak tam kapanmadı (en sık ve en kolay).** Kapı kilidi "klik" yapmazsa makine programı başlatmaz, çoğu modelde uyarı ışığı yanıp söner.

**2. Su almıyor.** Musluk kapalı, giriş filtresi tıkalı ya da hortum bükükse makine su bekler ve hata verir.

**3. Su atmıyor / tahliye sorunu.** Tahliye filtresi veya pompası tıkalıysa makine suyu boşaltamaz, deseni yanıp söner.

**4. Isıtma arızası.** Rezistans veya sensör arızasında ısıtma adımında takılır, hata gösterir.

**5. Dengesiz yük.** Tek ağır parça topaklanınca su atma adımına geçemez, uyarı verir.

**6. Elektronik kart arızası.** Yukarıdakiler yokken sürekli/anlamsız yanıp sönme karta işaret edebilir.

## Servisi aramadan önce kendin kontrol et

1. **Kapağı tam kapat.** "Klik" sesini duy; kapı iyi kapanmazsa program başlamaz.
2. **Su musluğunu ve giriş filtresini kontrol et.** Musluk açık mı, süzgeç tıkalı mı bak.
3. **Tahliye filtresini temizle.** Alt ön kapağı aç, filtreyi çıkar (su gelir, havlu hazırla), tüy/parçayı temizle.
4. **Yükü dengele.** Tek ağır parça varsa böl.
5. **Resetle.** Fişi 1 dakika çek, tekrar dene. Çocuk kilidi açık mı bak.
6. **Deseni not al.** Hangi ışık, kaç kez yanıyor — kod tablosuna ya da Benservis'e tarif et.

## Ne zaman mutlaka servis çağırmalısın?

- Kapı, su ve tahliye sağlamken **desen sürüyorsa** (kart/sensör)
- **Isıtma hata kodu** varsa (rezistans/sensör)
- Aynı kod resetlemeye rağmen **hemen geri geliyorsa**

## Tamir maliyeti ne kadar olur?

Bu arızanın maliyeti markaya, modele ve gerçek arızaya göre değişir; bu sayfada aralık vermiyoruz. **[Cihazına göre tahmini maliyeti ücretsiz öğren →](/)** Belirtiyi yaz, olası arızayı ve tahmini maliyeti saniyede gör.

## Sık sorulan sorular

**Işıklar neden yanıp söner?**
Genelde bir hata kodudur: kapı kilidi, su almama/atmama, ısıtma ya da denge sorununu desenle anlatır.

**Kendim çözebilir miyim?**
Kapağı kapat, filtreyi temizle, musluğu aç, resetle. Desen geçmiyorsa servis gerekir.

**Kodun anlamını nasıl öğrenirim?**
Kılavuzdaki tabloya bak ya da belirtiyi Benservis'e yaz; olası arızayı ve fiyatı söyler.

**Tamiri kaç para?**
Arızaya göre değişir; kesin tahmini cihazına göre [Benservis'ten ücretsiz al](/).
